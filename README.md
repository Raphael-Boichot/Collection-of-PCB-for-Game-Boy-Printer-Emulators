...
For this particular project you will have to recompile the source code to activate the external LED and the "paper tearing" functions.   LED is routed by default to GPIO25 which is the internal one (if any...) and "paper tearing" is routed by default to GPIO23 which is not reachable on a regular board...