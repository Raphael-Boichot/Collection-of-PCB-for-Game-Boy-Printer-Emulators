...
Want to buy a [Retrospy Pixel GameBoy Printer Emulator](https://retro-spy.com/product/pixel-gameboy-printer/) ? The product as you will get it from Retrospy is just a plain [Arduino Game Boy Printer emulator](https://github.com/mofosyne/arduino-gameboy-printer-emulator) (probably flashed on Arduino Nano) enclosed in a 3D printed shell. It has the same pinout, the same LED and the exact same firmware. This is the exact same device that you will make here after a bit of soldering.
