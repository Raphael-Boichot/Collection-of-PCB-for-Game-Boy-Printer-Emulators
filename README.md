...
You can modify and copy designs or sell these bare or populated PCBs, I don't care, I earn a decent enough salary not to go looking for pennies. Just cite me as author.
...
**Sidenote:**
This board id also partly compatible with the Arduino based projects if you compile them with the [Earle Philhower RP2040 core](https://github.com/earlephilhower/arduino-pico) installed on Arduino IDE. Just route LED to pin D8 instead of D13 and GBP_SO_PIN to D0 instead of D4. However, the serial output format is not exactly the same (not sure why, maybe just a serial baudrate issue) and the code needs a bit of rewrite to be directly compatible with the various image decoders. On the other hand, driving the printer with it seems working after a quick trial on my side. Anyway, I won't make any further try but it looks possible to use this board with other Game Boy Printer emulators without too much work.
