...
Want to buy a [Retrospy Pixel GameBoy Printer Emulator](https://retro-spy.com/product/pixel-gameboy-printer/) ? The product as you will get it from Retrospy is just a ~~scam~~ plain [Arduino Game Boy Printer emulator](https://github.com/mofosyne/arduino-gameboy-printer-emulator) enclosed in a 3D printed shell, coming along with a proprietary image converter. It has the same pinout and the exact same hardware/firmware as the original project. The PCB proposed here were tested with Retrospy softwares and are of course 100% compatible, for much cheaper.
